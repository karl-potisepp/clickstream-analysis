...
From \ref{frequent_patterns}, it can be seen that the root page of the web site (\emph{avaleht}) is one of the most frequently accessed resources, which is expected. The advantage of frequent pattern mining can be explained with the itemset where pages \emph{avaleht, 15208, inimesedInstituudid} are present together: it shows that navigation usually starts from the root page and also that the page containing links to different departments' contact information (\emph{inimesedInstituudid}) is used as a gateway to the page listing all the faculty members (\emph{15208}). The order in which the pages were navigated is not obvious, and looking at the web site we see that there do exist links to go either way. This problem can be solved by storing browsing sessions as sequences and mining frequent sequential patterns.
